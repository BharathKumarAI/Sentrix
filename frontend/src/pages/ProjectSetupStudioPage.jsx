...
export function ProjectSetupStudioPage({ activeProject, onProjectUpdated }) {
  ...
  const projectKey = (routeProjectKey || activeProject?.project_key || "BILLING").toUpperCase();
  ...
  const [config, setConfig] = useState({
    ...
    polling_jql: `project = "${projectKey}" AND (queue in ("BILLING-SRE-QUEUE", "PAYMENTS-GATEWAY-QUEUE", "CHECKOUT-ESCALATIONS") OR fixTeam = "Payments Core Team" OR assignee in ("sarah.k@company.com", "marcus.t@company.com")) AND status in ("Open", "In Progress", "Escalated") ORDER BY priority DESC`,
    auto_sync_jira: true,
    ...
    datasources: [
      { id: "pg-billing", name: "PostgreSQL Primary", type: "DATABASE", env: "prod", host: "billing-db-primary.internal:5432", status: "CONNECTED", latency: "14.2ms" },
      { id: "dd-logs", name: "Datadog Logs & APM", type: "OBSERVABILITY", env: "prod", host: "api.datadoghq.com/v2/logs", status: "CONNECTED", latency: "28.6ms" },
      { id: "k8s-cluster", name: "Kubernetes Cluster Operator", type: "COMPUTE", env: "prod", host: "k8s-prod-us-east-1:6443", status: "CONNECTED", latency: "18.1ms" },
      { id: "redis-grid", name: "Redis Session Grid", type: "CACHE", env: "prod", host: "redis-cluster-shard-01:6379", status: "CONNECTED", latency: "4.8ms" }
    ]
  });
  ...
  const loadConfiguration = async () => {
    ...
    try {
      ...
      if (data && typeof data === "object" && data.project_key) {
        // Normalize queues
        let queues = ["BILLING-SRE-QUEUE", "PAYMENTS-GATEWAY-QUEUE"];
  ...
  const handleTestDatasource = (dsId) => {
    setTestingDsId(dsId);
    setTimeout(() => {
  ...
  const safeQueues = Array.isArray(config?.jira_queues) && config.jira_queues.length > 0
    ? config.jira_queues
    : [config?.jira_queue || "BILLING-SRE-QUEUE"];
                  ...
                  <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
                    <div>
                      ...
                      <div style={{ fontSize: "11.5px", color: "var(--ink-tertiary)", marginTop: "2px" }}>
                        Active Env: <strong>{ds.env}</strong>
                      </div>
                    </div>
      ...
      {activeTab === "topology" && (
        <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <span className="badge badge-magenta">ADK 2.8 Graph</span>
          </div>
          ...
          >
            ...
            <div style={{ textAlign: "center", padding: "16px", borderRadius: "8px", background: "var(--bg-card)", border: "1px solid var(--prism-magenta)", minWidth: "180px", boxShadow: "0 0 15px var(--prism-glow)" }}>
              ...
              <div style={{ fontSize: "10.5px", color: "var(--accent-teal)" }}>Gemini 2.5 Pro (ADK 2.8)</div>
            </div>
          ...
          <div className="prism-card" style={{ padding: "22px", background: "var(--bg-card)", border: "1px solid var(--border-card)", display: "flex", flexDirection: "column", gap: "16px" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
              <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                ...
                <select
                  value={config?.model || "Gemini 2.5 Pro (ADK 2.8)"}
                  onChange={(e) => setConfig({ ...config, model: e.target.value })}
                  style={{ padding: "8px 12px", borderRadius: "6px", background: "var(--bg-input)", border: "1px solid var(--border-subtle)", color: "var(--ink-primary)", fontSize: "12px" }}
                >
                  <option value="Gemini 2.5 Pro (ADK 2.8)">Google Gemini 2.5 Pro (ADK 2.8 Default)</option>
